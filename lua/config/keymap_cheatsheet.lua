...
function M.generate_lines()
    ...
    for _, mode in ipairs(mode_order) do
        ...
        if #maps > 0 then
            ...
            for _, map in ipairs(maps) do
                table.insert(lines,
                    string.format("| `%s` | %s | %s | `%s` |", esc_cell(map.lhs), esc_cell(map.desc), map.scope,
...
vim.keymap.set("n", "<leader>fK", M.open, {
    desc = "Find: Generated Keymap Cheatsheet",
    silent = true,
    noremap = true
})
...
vim.keymap.set("n", "<leader>fW", function()
    ...
    desc = "Find: Write Keymap Cheatsheet",
    silent = true,
    noremap = true
})
