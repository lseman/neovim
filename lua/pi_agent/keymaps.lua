...
local function register_keymaps()
    ...
    map("v", "<C-p>", "<cmd>PiAgentAskSelection<CR>", vim.tbl_extend("force", default_opts, {
        desc = "pi-agent: ask about selection",
    }))

    -- Ask pi with current buffer context
    map("n", "<C-p>", "<cmd>PiAgentAsk<CR>", vim.tbl_extend("force", default_opts, {
        desc = "pi-agent: ask with context",
    }))

    -- Abort pi agent
    map("n", "<C-\\>", "<cmd>PiAgentAbort<CR>", vim.tbl_extend("force", default_opts, {
        desc = "pi-agent: abort",
    }))
end
