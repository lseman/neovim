return {
    ...
    opts = {
        ...
        on_attach = function(buf)
            ...
            local exclude_fts = {"help", "alpha", "dashboard", "snacks_picker_list", "Trouble", "lazy", "mason",
                                 "TelescopePrompt", "toggleterm", "spectre_panel"}
    ...
    config = function(_, opts)
        ...
        vim.api.nvim_create_autocmd("ColorScheme", {
            group = vim.api.nvim_create_augroup("TreesitterContextHl", {
                clear = true
            }),
        ...
        end, "Jump to Treesitter Context")

        map("<leader>tg", function()
            ctx.toggle()
            local enabled = ctx.enabled()
            vim.notify("Treesitter Context " .. (enabled and "enabled" or "disabled"),
        ...
        vim.api.nvim_create_user_command("TSContextRefresh", function()
            ...
            desc = "Refresh Treesitter Context display"
        })

        vim.api.nvim_create_user_command("CtxRefresh", "TSContextRefresh", {
            desc = "Alias for TSContextRefresh"
        })
    end
}
