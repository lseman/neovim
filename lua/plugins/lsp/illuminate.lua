return {
    ...
    keys = {{
        ...
        desc = "Illuminate: Toggle global"
    }, {
        "<leader>ht",
        function()
            require("illuminate").toggle_buf()
        end,
        desc = "Illuminate: Toggle buffer"
    }, {
        "<leader>hp",
        function()
            require("illuminate").pause()
        end,
        desc = "Illuminate: Pause"
    }, {
        "<leader>hr",
        function()
            require("illuminate").resume()
        end,
        desc = "Illuminate: Resume"
    }, {
        "<leader>hf",
        function()
    ...
    opts = {
        ...
        filetypes_denylist = {"dirvish", "fugitive", "NvimTree", "snacks_picker_list", "TelescopePrompt",
                              "TelescopeResults", "Trouble", "mason", "lazy", "alpha", "dashboard", "help", "qf",
                              "toggleterm", "harpoon", "DressingSelect", "notify", "noice", "WhichKey", "oil", "log",
    ...
    config = function(_, opts)
        ...
        if not ok or not illuminate then
            vim.notify("vim-illuminate: failed to load", vim.log.levels.WARN)
            return
        end

        -- Protect configure in case of unexpected plugin state
        if type(illuminate.configure) == "function" then
            illuminate.configure(opts)
        end

        -- ── Navigation keymaps (support count with vim.v.count1) ────────────────
        local function map(key, direction)
            local fn = illuminate["goto_" .. direction .. "_reference"]
        ...
        vim.api.nvim_create_autocmd("ColorScheme", {
            group = vim.api.nvim_create_augroup("IlluminateHighlightSetup", {
                clear = true
            }),
            callback = function()
        ...
        vim.api.nvim_create_autocmd("BufReadPost", {
            group = vim.api.nvim_create_augroup("IlluminateLargeFilePause", {
                clear = true
            }),
                ...
                if lines > opts.large_file_cutoff and not paused_large[buf] then
                    vim.defer_fn(function()
                        if vim.api.nvim_buf_is_valid(buf) then
                            pcall(function()
                                if type(illuminate.pause_buf) == "function" then
        ...
        vim.api.nvim_create_autocmd("BufWritePost", {
            group = vim.api.nvim_create_augroup("IlluminateLargeFileResume", {
                clear = true
            }),
            callback = function(args)
                local buf = args.buf
                if paused_large[buf] and vim.api.nvim_buf_line_count(buf) <= opts.large_file_cutoff then
                    pcall(function()
                        if type(illuminate.resume_buf) == "function" then
