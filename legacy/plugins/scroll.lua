-- Replaced by native vim.opt.smoothscroll = true (set in config/options.lua)
return {
    ...
    opts = {
        ...
        "WinScrolled", "CursorMoved", "CursorMovedI", "TextChanged", "TextChangedI"},

        -- Mappings: only half-page/line + z- commands (no full-page jumps)
        mappings = {"<C-u>", "<C-d>", "<C-y>", -- line-wise (optional but nice)
        "zt", "zz", "zb"},
    ...
    config = function(_, opts)
        ...
        local augroup = vim.api.nvim_create_augroup("NeoscrollCustom", {
            clear = true
        })
        ...
        vim.api.nvim_create_autocmd("FileType", {
            group = augroup,
            pattern = {"toggleterm", "terminal", "lazyterm", "dashboard", "alpha", "starter", "mini.starter", "lazy",
                       "mason", "lspinfo"},
            callback = function()
        ...
        vim.api.nvim_create_autocmd("FileType", {
            group = augroup,
            pattern = {"TelescopePrompt", "TelescopeResults", "neo-tree", "neo-tree-popup", "NeogitStatus", "Trouble",
                       "qf", "OverseerList", "spectre_panel", "WhichKey", "noice", "notify", "DressingInput",
                       "DressingSelect"},
            callback = function()
